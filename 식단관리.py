...
# 식품안전처 API 설정
# ============================================================
FOOD_API_KEY = "3a679e3067abee28622c0e2360e218fafd53ab6835a942352212b67bbddb2116"  # ← 발급받은 키 여기에 입력!
...
def search_food(food_name):
    ...
    try:
        ...
        if items:
            item = items[0]
            return {
                "음식명": item.get("FOOD_NM_KR", food_name),
                "칼로리": float(item.get("AMT_NUM1", 0) or 0),
                "탄수화물": float(item.get("AMT_NUM7", 0) or 0),
                "단백질": float(item.get("AMT_NUM3", 0) or 0),
                "지방": float(item.get("AMT_NUM4", 0) or 0),
            }
        return None
    except:
        return None
